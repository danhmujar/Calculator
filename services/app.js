...
class AppOrchestrator {
      ...
      {},
      {
        get: (_, prop) => {
          ...
          if (state.persistent && prop in state.persistent)
            return state.persistent[prop];
          if (state.transient && prop in state.transient)
            return state.transient[prop];
          return state[prop];
        },
        set: (_, prop, value) => {
          const state = store.getState();
  ...
  loadState() {
    ...
    try {
      ...
      uiManager.restoreState(effectiveState, {
        addAuditEntry: (a, b, op, res, expr) =>
          this.addAuditEntry(a, b, op, res, false, expr),
      });
  ...
  updateDisplay() {
    uiManager.updateDisplay(
      this.calcState,
      uiManager.formatOperator.bind(uiManager)
    );
  }

  syncScientificRows() {
    const rows = Array.from(document.querySelectorAll('math-field')).map(
      (mf) => mf.value
    );
  ...
  calcDigit(digit) {
    if (this.calcState.resetNext) {
      ...
      if (
        this.calcState.currentValue.replace(/[^0-9]/g, '').length >=
        this.INPUT_LENGTH_LIMIT
      )
        return;
      ...
      if (digit === '.') {
        ...
        if (
          ...
          this.calcState.currentValue = this.calcState.currentValue.startsWith(
            '-'
          )
            ? '-' + digit
            : digit;
  ...
  calcMemory(action) {
    ...
    if (action === 'MC') {
      ...
      uiManager.showToast(
        'Memory Recalled: ' +
          this.proFormatter.format(this.calcState.memoryValue)
      );
    } else if (action === 'M+') {
      if (!isNaN(val)) {
  ...
  calcEquals(logHistory = true) {
    if (this.calcState.operator && this.calcState.previousValue !== null) {
      ...
      const result = CalculatorService.evaluate(
        `a ${this.calcState.operator} b`,
        { a: prevNum, b: currentNum }
      );
  ...
  addAuditEntry(a, b, op, res, shouldSave = true, expr = null) {
    if (shouldSave) {
      ...
      if (this.auditEntries.length > this.MAX_AUDIT_ENTRIES)
        this.auditEntries.length = this.MAX_AUDIT_ENTRIES;
    }
    uiManager.addAuditEntry(
      a,
      b,
      op,
      res,
      uiManager.formatOperator.bind(uiManager),
      (val) => this.useAuditValue(val),
      expr
    );
  }
