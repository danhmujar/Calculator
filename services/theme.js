...
class ThemeTransitionManager {
  ...
  async init() {
    try {
      ...
      const response = await fetch(
        `${baseUrl}services/themes.json?t=${Date.now()}`
      );
  ...
  _fetchThemeColors() {
    ...
    const targetState = {
      ...
      uAuroraColor1: parseHexToRgb(
        themeData['--aurora-color-1'] || (isDark ? '#080c14' : '#f4f5f7')
      ),
  ...
  _applyColorsToDOM(state) {
      ...
      if (key.startsWith('--')) {
        if (Array.isArray(value)) {
          if (value.length === 4) {
            ...
            style.setProperty(
              key,
              `rgba(${Math.round(value[0])}, ${Math.round(value[1])}, ${Math.round(value[2])}, ${value[3]})`
            );
          } else {
            style.setProperty(
              key,
    ...
    if (state['--primary-blue'] && Array.isArray(state['--primary-blue'])) {
      ...
      style.setProperty(
        '--primary-alpha-05',
        `rgba(${rInt}, ${gInt}, ${bInt}, 0.05)`
      );
      style.setProperty(
        '--primary-alpha-10',
        `rgba(${rInt}, ${gInt}, ${bInt}, 0.10)`
      );
      style.setProperty(
        '--primary-alpha-15',
        `rgba(${rInt}, ${gInt}, ${bInt}, 0.15)`
      );
      style.setProperty(
        '--primary-alpha-30',
        `rgba(${rInt}, ${gInt}, ${bInt}, 0.30)`
      );
    }
  }
}
