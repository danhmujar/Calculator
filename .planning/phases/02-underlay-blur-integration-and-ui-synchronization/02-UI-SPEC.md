...
- **Corner Radius:** 
    ...
    - `12px`: Buttons (`.btn`, `.calc-btn`).
    - `16px`: Main display (`#main-calc-display`).
...
| `--primary-blue` | Accent Color | `uAccentColor` | 10% (Highlights) |
...
- **Result:** The blurred texture is rendered *only* behind DOM elements tracked by `layoutManager`.

## 3. Interaction Contract: Synchronization & Resizing

### Resizing Logic
- **Owner:** `WebGLRenderer` (via a dedicated `ResizeObserver`).
- **Priority:** Must trigger *before* the next animation frame after a DOM resize.
- **Sequence:**
...
- **A11y-02:** No interactive elements should be rendered *solely* in WebGL without a DOM backing for screen readers.
