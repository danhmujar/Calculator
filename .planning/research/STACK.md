...
- **CSS `backdrop-filter` combined with WebGL**: Do not mix them. Let CSS handle DOM placement and let WebGL handle _all_ pixel manipulations (blurs/gradients) to avoid triggering separate compositing layers in the browser engine, which tanks performance.
