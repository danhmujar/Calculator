...
1. **Z-Index Strategy**: The most lethal pitfall identified is stacking context resets caused by the scientific mode toggle. The roadmap _must_ dedicate a phase precisely to establishing a flat `z-index` sibling relationship between the DOM `<main>` layout and the `<canvas>`.
