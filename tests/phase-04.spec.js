...
test.describe('Phase 4: Optimization and UI Refinement Validation', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForFunction(
      () => window.uiManager && window.uiManager.themeManager
    );
  });
  ...
  test('UI-REF-01/02: Theme Transition Timing & Easing', async ({ page }) => {
    const transitionData = await page.evaluate(async () => {
      ...
      const auroraSwatch = document.querySelector(
        '.theme-swatch[data-theme="theme-aurora"]'
      );
      ...
      const sunsetSwatch = document.querySelector(
        '.theme-swatch[data-theme="theme-aurora-sunset"]'
      );
    ...
    if (totalDiff > 0.01) {
      const progressAtMid =
        Math.abs(mid.color1[0] - start.color1[0]) / totalDiff;
  ...
  test('UI-REF-03/04/05: Glass Opacity Audit', async ({ page }) => {
    const glassMetrics = await page.evaluate(() => {
      ...
      const glassVar = getComputedStyle(document.body)
  ...
  test('UI-REF-06: Eye Tracking Inertia (EMA)', async ({ page }) => {
    ...
    const isMonotonic = xValues.every(
      (v, i) => i === 0 || xValues[i - 1] <= v || xValues[i - 1] >= v
    );
