...
test.describe('Phase 5 Design System Compliance', () => {
  ...
  test('Typography Compliance', async ({ page }) => {
    const checkFontSize = async (selector, expectedPx, label) => {
        ...
        if (fontSize === 'NOT FOUND') throw new Error(`${label} element not found`);
  ...
  test('Spacing Scale Compliance (Multiples of 4)', async ({ page }) => {
    ...
    expect(isMultipleOf4(aboutModalInnerPadding.top), `Padding top ${aboutModalInnerPadding.top} should be multiple of 4`).toBe(true);
    expect(isMultipleOf4(aboutModalInnerPadding.left), `Padding left ${aboutModalInnerPadding.left} should be multiple of 4`).toBe(true);
    expect(isMultipleOf4(aboutModalInnerPadding.right), `Padding right ${aboutModalInnerPadding.right} should be multiple of 4`).toBe(true);
    expect(isMultipleOf4(aboutModalInnerPadding.bottom), `Padding bottom ${aboutModalInnerPadding.bottom} should be multiple of 4`).toBe(true);
  });
