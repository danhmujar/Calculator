...
export class WebGLRenderer {
  ...
  constructor(webglContext, typography = null) {
    ...
    this.context.onRestored = () => {
      console.info(
        'WebGLRenderer: Context restored, re-initializing pipeline...'
      );
      this.init();
      this.render();
    };
  ...
  init() {
    try {
      ...
      this.batchProgram = ShaderManager.createProgram(
        gl,
        BATCH_VERT,
        BATCH_FRAG
      );
      this.batchVAO = BufferManager.createInstancedVAO(
        gl,
        this.quadData,
        this.maxInstances
      );
      ...
      this.primitiveProgram = ShaderManager.createProgram(
        gl,
        PRIMITIVE_VERT,
        PRIMITIVE_FRAG
      );
      ...
      const primIndex = gl.getUniformBlockIndex(
      ...
      console.info(
        'WebGLRenderer: Instanced batch rendering pipeline initialized.'
      );
  ...
  getTransitionData(id, currentRect, currentColor, durationMs = 250) {
    ...
    const rectChanged =
      Math.abs(history.endRect.x - currentRect.x) > 0.1 ||
    ...
    if (rectChanged || colorChanged) {
      ...
      const t =
        history.duration > 0
          ? Math.min(1.0, (now - history.startTime) / history.duration)
          : 1.0;
      ...
      const currentInterpRect = {
        x:
          history.startRect.x +
          (history.endRect.x - history.startRect.x) * easedT,
        y:
          history.startRect.y +
          (history.endRect.y - history.startRect.y) * easedT,
        width:
          history.startRect.width +
          (history.endRect.width - history.startRect.width) * easedT,
        height:
          history.startRect.height +
          (history.endRect.height - history.startRect.height) * easedT,
      };

      const currentInterpColor = [
        history.startColor[0] +
          (history.endColor[0] - history.startColor[0]) * easedT,
        history.startColor[1] +
          (history.endColor[1] - history.startColor[1]) * easedT,
        history.startColor[2] +
          (history.endColor[2] - history.startColor[2]) * easedT,
        history.startColor[3] +
          (history.endColor[3] - history.startColor[3]) * easedT,
      ];
  ...
  render() {
    ...
    if (
      this.fboA &&
      (this.fboA.width !== Math.max(1, Math.floor(gl.canvas.width / 4)) ||
    ...
    if (
      theme.uBackgroundMode === 2 &&
      this.btsBackgroundTex &&
      this.btsTexLoaded
    ) {
  ...
  _drawBlurredStage() {
    ...
    for (const [element, id] of layoutManager.elements.entries()) {
      ...
      if (element.classList.contains('calc-card')) {
        ...
        element.classList.contains('math-row') ||
        element.classList.contains('calc-row-instance')
      ) {
        this.pushRect(
          rect,
          [...this.themeColors.primary, isHovered ? 0.1 : 0.05],
          8,
          id
        );
      }
