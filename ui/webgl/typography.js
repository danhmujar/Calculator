...
export class TypographyManager {
  constructor() {
    ...
    if (sciContainer) {
      sciContainer.addEventListener('scroll', () => this.scheduleUpdate(), {
        passive: true,
      });
    }
  ...
  update() {
    ...
    const containerRect = sciContainer
      ? layoutManager.getRect(sciContainer)
      : null;
      ...
      if (
        ...
        if (containerRect) {
          glyphs.forEach((g) => {
            ...
            const padding = 20;
            if (
              g.y + g.height >= containerRect.top - padding &&
  ...
  extractGlyphs(mf) {
    ...
    const root =
      mf.shadowRoot.querySelector('.ML__fieldcontainer__field') ||
    ...
    while ((node = walker.nextNode())) {
      ...
      if (
        parent.classList.contains('ML__caret') ||
